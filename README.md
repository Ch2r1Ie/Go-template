...
- set up GOPRIVATE environment variable to allow go get private repository
  - `go env -w GOPRIVATE=gitdev.devops.com`
- config ssh for private repo [GitLab SSH Keys](https://gitdev.devops.com/-/profile/keys). otherwise, you will not be able to install use `gonew` command to create a new project.
  - or use personal access token set in `~/.netrc` file

```.netrc
machine gitdev.devops.com
login <account name>
...
```git
[url "ssh://git@gitdev.devops.com:2222/"]
        insteadOf = https://gitdev.devops.com/
```
...
1. make sure you set `go env -w GOPRIVATE=gitdev.devops.com` (to allow go get private repository)
1. copy this repository and rename it to your project name or use `gonew gitdev.devops.com/techcoach/template/simple-ms-template.git gitdev.devops.com/your/service/module/xxxx.git`
...
1. add your git remote repository by running `git remote add origin gitdev.devops.com/your/service/module/xxxx.git` (DO NOT FORGET TO CREATE YOUR REPOSITORY FIRST IN GitLab)
